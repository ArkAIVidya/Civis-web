...
import '../model/civics_live_data.dart';

part 'civics_data_manager.g.dart';

@riverpod
CivicsDataManager civicsDataManager(Ref ref) {
  return CivicsDataManager();
}

class CivicsDataManager {
  Future<CivicsLiveData?> getCivicsData() async {
    try {
      final jsonString = await rootBundle.loadString(
        'assets/civics_live_data.json',
      );
      final Map<String, dynamic> jsonMap = json.decode(jsonString);
      return CivicsLiveData.fromJson(jsonMap);
    } catch (e) {
      // Use a proper logger in production
      debugPrint('Error loading civics data: $e');
      return null;
    }
  }

  String? getStateAbbreviation(String zipCode) {
    if (zipCode.length < 3) return null;
    final prefix = int.tryParse(zipCode.substring(0, 3));
